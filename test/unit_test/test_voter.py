...
class TestVoter(unittest.TestCase):
    def test_vote(self):
        template_voter = Voter(
            label_map={"Yes": 1, "No": 2}, matching_fn=lambda x, y: x == y
        )
        ...
        response = CompletionResponse("no")
        votes = template_voter.vote(
            response, matching_function=lambda x, y: x.lower() == y.lower()
        )
        self.assertEqual(len(votes), 1)
        self.assertEqual(votes[0], 2)
    ...
    def test_calibrated_vote(self):
        ...
        response = RankedResponse(
            "Yes", logits={"Yes": 0.9, "No": 0.1}, scores={"Yes": 0.9, "No": 0.1}
        )
        votes = template_voter.vote(response)
        self.assertEqual(len(votes), 1)
        self.assertEqual(votes[0], 1)
