...
class TestGRPCServer(unittest.TestCase):
    ...
    def test_run_single_query(self):
        ...
        response = self.stub.Inference(query__pb2.InferenceRequest(message=query.prompt))
        print(response)
    ...
    def test_run_multiple_queries(self):
        ...
        _ = self.stub.DataHeader(query__pb2.DataHeaderRequest(data_size=len(queries)))
        for query in queries:
            _ = self.stub.Inference(query__pb2.InferenceRequest(message=query.prompt))
        responses = self.stub.DataReady(query__pb2.DataReadySignal(data_size=len(queries)))
