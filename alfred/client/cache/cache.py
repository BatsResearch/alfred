...
class Cache(abc.ABC):
    ...
    @abc.abstractmethod
    def write_batch(self, prompts: List[str], response: List[str], metadata: Optional[str]=None):
        """
        ...
        def run_query(queries: Union[Response, List[Response], str, List[str]],
            ...
            try:
                ...
                for q_idx, query in enumerate(queries):
                    ...
                    cached_response = self.read(query.serialize() if isinstance(query, Query)
                                                else str(query), metadata)
                    if cached_response:
            ...
            if len(new_q_idx) > 0:
                ...
                _model_responses = model_run(_new_queries[0] if len(
                    _new_queries) == 1 else _new_queries, **kwargs)
                ...
                try:
                    ...
                    for query, response in zip(
                        ...
                        self.write(
                            query, response, metadata=metadata)
